...
def generate_launch_description():
    ...
    view_amcl_in_rviz = LaunchConfiguration('view_amcl_in_rviz')
    ...
    declare_view_amcl_in_rviz_cmd = DeclareLaunchArgument(
        'view_amcl_in_rviz',
    ...
    declare_map_cmd = DeclareLaunchArgument(
        'map',
        # default_value='',
        default_value=os.path.join(my_nav_pkg, 'maps', 'my_test_map.yaml'),
        description='Full path to map file to load')

    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(my_nav_pkg, 'config', 'my_nav2_bringup_params.yaml'),
    ...
    view_robot = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [os.path.join(my_robot_view_pkg,'launch','localize_with_amcl.launch.py')]
            ), 
            condition=IfCondition(view_amcl_in_rviz)
    )
    ...
    ld.add_action(declare_view_amcl_in_rviz_cmd)
